...
function writeLauncher(path, installedRoot, configuredStateRoot) {
  ...
  if (process.platform === "win32") {
    writeFileSync(path, `@echo off\r\nrem CUTAGENT_STANDALONE_LAUNCHER_V1\r\nset "CUTAGENT_SDK_STATE_DIR=${configuredStateRoot}"\r\n"${process.execPath}" "${nodeEntry}" %*\r\n`, "utf8");
  } else {
    const quote = value => "'" + value.replaceAll("'", "'\"'\"'") + "'";
    writeFileSync(path, `#!/bin/sh\n# CUTAGENT_STANDALONE_LAUNCHER_V1\nexport CUTAGENT_SDK_STATE_DIR=${quote(configuredStateRoot)}\nexec ${quote(process.execPath)} ${quote(nodeEntry)} "$@"\n`, {encoding: "utf8", mode: 0o755});
  }
}
...
function setup() {
  ...
  try {
    ...
    writeLauncher(commandPath, destination, root);
...
async function startRuntime() {
  ...
  const close = async () => {
    ...
    if (!runtime) return;
    closePromise ??= (async () => {
      try { await runtime.close(); } finally { await broker?.close(); }
    })();
    return closePromise;
  };
  ...
  try {
    if (transport === "embedded_free") {
      const {ensureFreeBroker} = await import(pathToFileURL(join(root, "runtime", "local", "free-broker.mjs")));
      broker = await ensureFreeBroker({python: join(root, ".venv", "bin", "python"), script: join(root, "native", "free_broker.py")});
    }
    if (shutdownRequested) { await broker?.close(); return; }
    runtime = await startNativeLocalRuntime({stateDirectory: runtimeState, transport});
    if (shutdownRequested) { await close(); return; }
  } catch (error) {
    await broker?.close();
    fail("RUNTIME_START_FAILED", error.message, "Check the local runtime dependencies and Free broker, then retry.");
    return;
  }
  emit(true, {message: "CutAgent SDK runtime is ready. Verify the DaVinci Resolve connection with cutagent --json status.", discoveryFile: runtime.discoveryFile, transport, resolveConnectionVerified: false,
    ...(transport === "embedded_free" ? {nextStep: "In DaVinci Resolve, run Workspace > Scripts > CutAgentSDK.", brokerOwned: broker.owned} : {})});
}
