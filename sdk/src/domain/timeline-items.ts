...
import {
  ...
  sdkTimelineClipMarkerBatchInputSchema,
...
/** A marker on an exact V1 clip with a proven normal forward time map, measured from its visible start. @beta */
export interface TimelineClipMarkerChange {
  readonly clip: ClipSnapshot;
...
/** Native-readback result of a bounded clip-marker batch. @beta */
export interface TimelineClipMarkerResult {
...
export interface TimelineItems {
  ...
  /** Insert markers on ordinary V1 clips; retimed/reversed or unproven time maps, mixed frame rates, and conflicting markers are rejected. */
  addMarkers(changes: readonly TimelineClipMarkerChange[], options: TimelineItemMoveOptions): Promise<OperationHandle<TimelineClipMarkerResult, "cutagent.action.timeline.clip_markers.batch">>;
...
const clipMarkerResultSchema = z.object({
  actionId: z.literal("cutagent.action.timeline.clip_markers.batch"),
  markers: z.array(z.object({
    clipId: TimelineItemIdSchema,
  ...
  return freeze({
    ...
    async addMarkers(changes: readonly TimelineClipMarkerChange[], options: TimelineItemMoveOptions) {
      ...
      const updates = changes.map(entry => {
        const matches = rows.filter(({clip}) => clip.id !== null && clip.id === entry.clip.id);
        const match = matches[0];
        if (matches.length !== 1 || !match || !ownsClip(entry.clip) || entry.clip.snapshotRevision !== snapshot.revision
          || entry.clip.snapshotId !== match.clip.snapshotId || entry.clip.name !== match.clip.name) throw new TypeError("Clip markers require exact V1 clips from the current snapshot.");
        const descriptor = clipDescriptor(match.clip, match.track.index, snapshot.frameRate);
        if (!Number.isSafeInteger(entry.offsetFrames) || entry.offsetFrames < 0 || !Number.isSafeInteger(entry.durationFrames)
          || entry.durationFrames < 1 || entry.offsetFrames + entry.durationFrames > descriptor.recordEndFrame - descriptor.recordStartFrame) throw new TypeError("Clip marker range escapes the selected clip.");
        const position = `${descriptor.id}:${entry.offsetFrames}`;
        if (positions.has(position)) throw new TypeError("Clip marker batch contains a duplicate position.");
        positions.add(position);
        return {timelineItemId: descriptor.id, snapshotTimelineItemId: descriptor.snapshotId,
          trackType: "video" as const, trackIndex: 1 as const, recordStartFrame: descriptor.recordStartFrame,
          recordEndFrame: descriptor.recordEndFrame, name: descriptor.name,
          offsetFrames: entry.offsetFrames, durationFrames: entry.durationFrames, color: entry.color, markerName: entry.name, note: entry.note};
      });
      const input = sdkTimelineClipMarkerBatchInputSchema.parse({projectId: String(snapshot.projectId),
        timelineId: String(snapshot.timelineId), revision: String(snapshot.revision), updates});
      const request: CarrierOperationRequest = {operation: "operation.create", actionId: "cutagent.action.timeline.clip_markers.batch",
        input, idempotencyKey: sdkIdempotencyKeySchema.parse(options.idempotencyKey)};
      const event = await runtime.createOperationAtGeneration(generation, request, options);
      return createTypedOperationHandle({session: () => runtime.sessionAtGeneration(generation)}, event,
        "cutagent.action.timeline.clip_markers.batch", clipMarkerResultSchema);
    },
