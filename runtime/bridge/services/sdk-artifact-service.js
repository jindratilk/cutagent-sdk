...
import {fstatFileIdentity, lstatFileIdentity, isFileIdentityComponent} from "./file-identity.js";
...
const DURABLE_READ_FLAGS = process.platform === "win32" ? fs.constants.O_RDWR : fs.constants.O_RDONLY;
...
function validateRecord(record, artifactId, root) {
  ...
  if (!record || typeof record !== "object"
    ...
    || !isFileIdentityComponent(record.dev) || !isFileIdentityComponent(record.ino)
    ...
    || (kind === "directory" && (
      ...
      || !record.directoryIdentity || !isFileIdentityComponent(record.directoryIdentity.dev) || !isFileIdentityComponent(record.directoryIdentity.ino)
...
function privateDirectorySnapshot(directoryPath, reservation) {
  const rootBefore = lstatFileIdentity(directoryPath);
  ...
  const visit = (current, relativeRoot) => {
    for (const name of fs.readdirSync(current).sort()) {
      ...
      const pathStat = lstatFileIdentity(absolute);
      ...
      if (pathStat.isDirectory()) {
        ...
        const after = lstatFileIdentity(absolute);
      ...
      try {
        const opened = fstatFileIdentity(descriptor);
        ...
        const after = fstatFileIdentity(descriptor);
        const afterPath = lstatFileIdentity(absolute);
  ...
  visit(directoryPath, "");
  const rootAfter = lstatFileIdentity(directoryPath);
...
function materializeDirectoryBundle(directoryPath, snapshot, bundlePath) {
  ...
  try {
    ...
    for (const entry of snapshot.entries) {
      ...
      try {
        const before = fstatFileIdentity(descriptor);
        const expected = {
        ...
        if (!sameIdentity(fstatFileIdentity(descriptor), expected)) throw new Error("SDK directory artifact changed during bundle publication.");
      } finally {
    ...
    fs.fsyncSync(output);
    const stat = fstatFileIdentity(output);
...
export function createSdkArtifactService({
  ...
  const assertRecordFile = (record) => {
    ...
    const stat = fstatFileIdentity(descriptor);
  ...
  const assertRecordDirectory = (record) => {
    ...
    const stat = lstatFileIdentity(record.directoryPath);
  ...
  const cleanup = () => {
    ...
    for (const [artifactId, record] of Object.entries(state.artifacts)) {
      ...
      if (record.kind === "directory") {
        try {
          const directoryStat = lstatFileIdentity(record.directoryPath);
      ...
      try {
        const stat = lstatFileIdentity(record.filePath);
    ...
    for (const [artifactId, reservation] of Object.entries(state.reservations)) {
      ...
      try {
        const stat = lstatFileIdentity(reservation.filePath);
  ...
  const reservePrivateFile = ({artifactId, operationId, accountFingerprint, extension}, purpose) => {
    ...
    if (existing) {
      ...
      const stat = lstatFileIdentity(existing.filePath);
  ...
  const reservePrivateDirectory = ({artifactId, operationId, accountFingerprint}, purpose) => {
    ...
    const stat = lstatFileIdentity(directoryPath);
  ...
  return Object.freeze({
    ...
    discardPrivateOwnedArtifact({artifactId, operationId, accountFingerprint, receiptSha256}) {
      ...
      const removeFile = (filePath, identity, label) => {
        const stat = lstatFileIdentity(filePath);
      ...
      if (source.kind === "directory") {
        ...
        const directoryStat = lstatFileIdentity(directoryPath);
    ...
    abortPrivateArtifactReservation({artifactId, operationId, accountFingerprint, identity}) {
      ...
      if (!ACCOUNT_FINGERPRINT_PATTERN.test(accountFingerprint ?? "")
        || !identity || !isFileIdentityComponent(identity.device) || !isFileIdentityComponent(identity.inode)) {
      ...
      if (source.operationId !== operationId || source.accountFingerprint !== accountFingerprint
        || String(source.dev) !== String(identity.device) || String(source.ino) !== String(identity.inode) || source.kind === "directory") {
        throw new Error("SDK private artifact abort ownership changed.");
      }
      const stat = lstatFileIdentity(source.filePath);
    ...
    publishPrivateOutputArtifacts({operationId, accountFingerprint, publicResult}) {
      ...
      for (const descriptor of publicDescriptors) {
        ...
        try {
          if (sha256Descriptor(referencedDescriptor, referenced.sizeBytes) !== descriptor.sha256
            || !sameIdentity(fstatFileIdentity(referencedDescriptor), referenced)) {
      ...
      try {
        for (const reservation of reservations) {
          ...
          const exact = matches[0];
          assertCanonicalPrivateDirectory(path.dirname(record.filePath), {label: "Managed render operation directory"});
          const descriptor = fs.openSync(record.filePath, DURABLE_READ_FLAGS | (fs.constants.O_NOFOLLOW ?? 0));
          const stat = fstatFileIdentity(descriptor);
          const pathStat = lstatFileIdentity(record.filePath);
        ...
        for (const artifactId of publicArtifactIds) {
          ...
          if (existing) {
            ...
            try {
              if (sha256Descriptor(descriptor, existing.sizeBytes) !== existing.sha256
                || !sameIdentity(fstatFileIdentity(descriptor), existing)) {
          ...
          const pathStat = lstatFileIdentity(filePath);
          const descriptor = fs.openSync(filePath, DURABLE_READ_FLAGS | (fs.constants.O_NOFOLLOW ?? 0));
          const stat = fstatFileIdentity(descriptor);
          if (!stat.isFile() || pathStat.isSymbolicLink() || pathStat.dev !== stat.dev || pathStat.ino !== stat.ino
        ...
        for (const candidate of candidates) {
          ...
          const after = fstatFileIdentity(descriptor);
          const afterPath = lstatFileIdentity(record.filePath);
        ...
        for (const candidate of adoptedCandidates) {
          ...
          const after = fstatFileIdentity(candidate.descriptor);
          const afterPath = lstatFileIdentity(candidate.filePath);
    ...
    capturePrivateManagedArtifact({artifactId, accountFingerprint}) {
      ...
      if (!record && reservation?.kind === "file" && reservation.purpose === "private"
        ...
        try {
          const stat = fstatFileIdentity(descriptor);
          const pathStat = lstatFileIdentity(reservation.filePath);
    ...
    capturePrivateManagedDirectory({artifactId, accountFingerprint, singleDirectorySuffix = null, singleDirectoryName = null}) {
      ...
      const reservationBefore = lstatFileIdentity(sourcePath);
      ...
      const capturedStat = lstatFileIdentity(capturedPath);
      ...
      const reservationAfter = lstatFileIdentity(sourcePath);
    ...
    register({ artifactId, operationId, accountFingerprint, filePath, sizeBytes, sha256 }) {
      ...
      const noFollow = Number.isInteger(fs.constants.O_NOFOLLOW) ? fs.constants.O_NOFOLLOW : 0;
      const descriptor = fs.openSync(absolute, DURABLE_READ_FLAGS | noFollow);
      try {
        const stat = fstatFileIdentity(descriptor);
        const pathStat = lstatFileIdentity(absolute);
        const reservation = state.reservations[artifactId];
