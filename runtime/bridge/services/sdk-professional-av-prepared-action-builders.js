...
import {z} from "zod";
import {captureWaveformProjectSources} from "./sdk-waveform-source-custody.js";
...
function exactPublicTimelineTargets(input) {
  return [
    ...
    ...transitionBatchEntries(input).flatMap((transition) => [
      transition.target, transition.outgoing, transition.incoming, ...(transition.linkedAudioTargets ?? []),
    ]),
...
function assertResidualEditHandlerScope(actionId, input, snapshot, rows) {
  ...
  if (actionId === "cutagent.action.edit.transition.batch") {
    ...
    for (const transition of transitionBatchEntries(input)) {
      if (transition.kind === "clip_edges") {
        const target = rows.find(({clip}) => clip.id === transition.target.id);
        if (!target || target.track.type !== "video" || transition.scope !== "video"
          || edgeIds.has(target.clip.id) || transition.durationFrames > target.clip.recordRange.endExclusive - target.clip.recordRange.start) {
          throw new Error("Prepared clip-edge transition target is invalid, duplicated or too short.");
        }
        edgeIds.add(target.clip.id);
        continue;
      }
...
function managedRoots(secretDir) {
  ...
  return [...new Set(selected.map((value) => path.resolve(value)))];
}
...
async function captureExactBinding(actionId, input, context, scope, minimum, dependencies) {
  ...
  const waveformSources = actionId === "cutagent.action.audio.waveform_offset"
    && inputPaths.some((file) => !managedRoots(dependencies.secretDir).some((root) => path.resolve(file) === root || path.resolve(file).startsWith(`${root}${path.sep}`)))
    ? await captureWaveformProjectSources({liveInspectionService: dependencies.liveInspectionService, projectId, paths: inputPaths}) : null;
  for (const [role, rawPath] of pathBindings) {
    ...
    } else if (waveformSources) {
      artifact = waveformSources.find((source) => source.resolvedPath === rawPath);
      if (!artifact) throw new Error("Waveform source lacks captured project-media custody.");
    } else {
    ...
    const payloadDigest = artifact.allowedRootId === "exact_project_media_source"
      ? artifact.sourceIdentityDigest : artifact.contentDigest;
    if (payloadDigest) referencedPayloadDigests.push(payloadDigest);
  ...
  if (actionId === "cutagent.action.system.keyframe_mode.set") {
    ...
    if (["cutagent.action.edit.transition.add", "cutagent.action.edit.transition.batch"].includes(actionId)) {
      ...
      if (actionId === "cutagent.action.edit.transition.add") {
        ...
        exactTransitionBatchTargets = transitionBatchEntries(input).map((transition) => transition.kind === "clip_edges" ? ({
          kind: "clip_edges", target: privateTarget(transition.target), scope: "video",
          placement: transition.placement, durationFrames: transition.durationFrames, transitionType: transition.transitionType,
        }) : ({
  ...
  return {
    ...
    privateContext: {
      ...
      ...(waveformSources ? {waveformSources} : {}),
