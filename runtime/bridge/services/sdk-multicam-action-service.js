...
const RESIDUAL_ACTIONS = Object.freeze({
  ...
  "cutagent.action.multicam.reorder_angles": { action: "reorder_angles", schema: target({ angleIds: z.array(sdkMulticamAngleIdSchema).min(2), includeAudio: z.boolean(), renameTracks: z.boolean(), strict: z.boolean() }).refine((input) => new Set(input.angleIds).size === input.angleIds.length, { message: "Angle identities must be unique" }) },
  ...
  "cutagent.action.multicam.recover_timing": { action: "recover_timing", schema: target({ sources: z.array(z.object({ mediaPoolItemId: sdkMediaPoolItemIdSchema, sourceStartFrame: frame, durationFrames: positiveFrame }).strict()).min(1), applyTimeMap: z.boolean(), applySourceStartTimecode: z.boolean(), applyMediaExtents: z.boolean(), verifyReopen: z.boolean() }).refine((input) => input.applyTimeMap || input.applySourceStartTimecode || input.applyMediaExtents, { message: "At least one timing repair must be selected" }) },
  "cutagent.action.multicam.replace.audio": { action: "replace.audio", schema: target({ replacements: z.array(audioReplacement).min(1), unmappedPolicy: z.enum(["keep", "remove", "error"]), allowShortSource: z.literal(false) }) },
  "cutagent.action.multicam.replace.video": { action: "replace.video", schema: target({ replacements: z.array(replacement).min(1), unmappedPolicy: z.literal("keep"), allowShortSource: z.boolean() }) },
  ...
  "cutagent.action.multicam.smart_switch": { action: "smart_switch", schema: target({ timelineId: sdkTimelineIdSchema, timelineRevision: sdkRevisionSchema,
    audioSources: z.array(z.object({ mediaPoolItemId: sdkMediaPoolItemIdSchema, angleId: sdkMulticamAngleIdSchema }).strict()).min(1),
    ...
    videoSourceOffsets: z.array(z.object({ angleId: sdkMulticamAngleIdSchema, offsetFrames: z.number().int().safe() }).strict()),
...
const settingsSchema = z.object({ projectId: sdkProjectIdSchema, mediaPoolRevision: sdkRevisionSchema, name: z.string().min(1).max(1024), timelineName: z.string().min(1).max(1024),
  sources: z.array(z.object({ mediaPoolItemId: sdkMediaPoolItemIdSchema, angleLabel: z.string().min(1).max(256) }).strict()).min(2),
...
const settingsResultSchema = z.object({ actionId: z.literal("cutagent.action.multicam.settings"), projectId: sdkProjectIdSchema, mediaPoolRevision: sdkRevisionSchema,
  effectiveSettings: z.object({ multicamName: z.string().min(1).max(1024), timelineName: z.string().min(1).max(1024), angleOrder: z.array(z.string().min(1).max(256)).min(2),
...
const calibrationSchema = z.object({ projectId: sdkProjectIdSchema, mediaPoolRevision: sdkRevisionSchema,
  videoSources: z.array(z.object({ mediaPoolItemId: sdkMediaPoolItemIdSchema, angleLabel: z.string().min(1).max(256) }).strict()).min(2),
  audioSources: z.array(z.object({ mediaPoolItemId: sdkMediaPoolItemIdSchema, angleLabel: z.string().min(1).max(256) }).strict()).min(1),
