...
function expectedResult(before, after, expected, nativeMulticam = false) {
  ...
  const matches = (track?.clips ?? []).filter((clip) => clip.recordRange.start === expected.recordRange.start
    ...
    && (clip.name === expected.name || (nativeMulticam && expected.role === "replacement"
      && expected.beforeItemId === null && expected.track.type === "video"
      && clip.name.startsWith(`${expected.name} - `)
      && clip.name.slice(expected.name.length + 3).trim().length > 0))
...
export function createSdkTimelineEditActions({
  ...
  return Object.fromEntries(Object.entries(ACTIONS).map(([actionId, action]) => [actionId, {
    ...
    async execute(context, rawInput) {
      ...
      const expectedResults = impacts.map((impact, index) => impact.expectedItems.map((expected) => expectedResult(
        prepared.snapshot, after, expected, prepared.executionContexts?.[index]?.nativeMulticam === true,
      )));
