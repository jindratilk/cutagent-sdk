...
export function renderDiscoverySupports(discovery, input) {
  ...
  // Some Free builds expose codecs but cannot enumerate resolutions. That is
  // unknown resolution support, not an unsupported codec. Native settings/job
  // readback and final artifact dimensions remain mandatory below.
  if (codec.resolutionSupport?.availability === "unavailable"
    && codec.resolutionSupport.reason === "api_unavailable") return true;
  ...
  const execute = async (context, rawInput) => {
    ...
    try {
      ...
      try {
        ...
        if (!renderDiscoverySupports(discovery, input)) {
