...
import {isFileIdentityComponent} from "./file-identity.js";
...
export const TIMELINE_VERSION_PREPARED_ACTION_IDS = Object.freeze([
  "cutagent.action.timeline.clip_color.batch", "cutagent.action.timeline.clip_markers.batch", "cutagent.action.timeline.clip_markers.list", "cutagent.action.timeline.current_item",
...
const TIMELINE_ORDINARY_MUTATION_ACTIONS = new Set([
  "cutagent.action.timeline.clip_color.batch", "cutagent.action.timeline.clip_markers.batch", "cutagent.action.timeline.create", "cutagent.action.timeline.delete", "cutagent.action.timeline.dolby.analyze",
  "cutagent.action.timeline.duplicate", "cutagent.action.timeline.fairlight_preset.apply", "cutagent.action.timeline.import",
  "cutagent.action.timeline.mark.clear", "cutagent.action.timeline.mark.set", "cutagent.action.timeline.playhead.set",
  "cutagent.action.timeline.rename", "cutagent.action.timeline.set_start_tc", "cutagent.action.timeline.settings_set", "cutagent.action.timeline.output_blanking.set",
  "cutagent.action.timeline.start_tc", "cutagent.action.timeline.switch",
...
function captureInputFile(input, secretDir, operationId) {
  ...
  // DaVinci Resolve selects its timeline importer from the file extension.
  // Keep only a bounded extension, never the user-controlled source basename.
  const sourceExtension = path.extname(sourcePath);
  const stagedExtension = /^\.[a-z0-9]{1,16}$/i.test(sourceExtension) ? sourceExtension : ".timeline-input";
  const stagedPath = path.join(stagingDir, `${operationId}${stagedExtension}`);
...
async function captureTimelineOutputArtifacts(actionId, input, artifactService, context) {
  ...
  for (const request of requests) {
    ...
    if (reserved?.artifactId !== request.artifactId || typeof reserved.absolutePath !== "string"
      || !reserved.identity || !isFileIdentityComponent(reserved.identity.device) || !isFileIdentityComponent(reserved.identity.inode)) {
...
async function captureTimeline(actionId, input, context, dependencies) {
  ...
  const requestedItemIds = actionId === "cutagent.action.timeline.dolby.analyze"
    ...
    : ["cutagent.action.timeline.clip_color.batch", "cutagent.action.timeline.clip_markers.batch"].includes(actionId)
      ? [...new Set(input.updates.map((update) => update.timelineItemId))]
      : null;
  if (requestedItemIds) {
    if (actionId === "cutagent.action.timeline.clip_markers.batch") {
      const snapshot = timelineRead.value;
      const rows = (snapshot.tracks ?? []).flatMap((track) => (track.clips ?? []).map((clip) => ({track, clip})));
      if (input.revision !== snapshot.revision) throw new Error("Clip marker snapshot revision changed.");
      for (const update of input.updates) {
        const matches = rows.filter(({clip}) => clip.id === update.timelineItemId);
        if (matches.length !== 1) throw new Error("Clip marker snapshot identity is missing or ambiguous.");
        const {track, clip} = matches[0];
        if (track.type !== "video" || track.index !== 1 || clip.snapshotId !== update.snapshotTimelineItemId
          || clip.name !== update.name || clip.recordRange.start !== update.recordStartFrame
          || clip.recordRange.endExclusive !== update.recordEndFrame) throw new Error("Clip marker snapshot target changed.");
      }
    }
...
async function captureFusion(actionId, input, context, dependencies) {
  ...
  const exactCompositionInput = Boolean(input.timelineItemId && input.compositionIndex);
  ...
  if (input.timelineId) {
    // Single-composition actions carry the Fusion composition revision in
    // `input.revision`, not the enclosing Timeline revision. Capture the live
    // Timeline independently, then bind and validate the exact composition
    // below against that current snapshot.
    timelineRead = await liveTimeline(
      dependencies.liveInspectionService,
      exactCompositionInput ? {...input, revision: undefined} : input,
    );
  }
  ...
  if (actionId === "cutagent.action.fusion.image.batch") {
    ...
    const references = await dependencies.liveInspectionService.read({
      operation: "fusion.compositions", projectId: input.projectId, timelineId: input.timelineId,
      timelineItemId: input.timelineItemId, expectedRevision: timelineRead?.value?.revision ?? null,
    });
    const reference = references?.find((row) => row.index === input.compositionIndex);
    if (!reference || (input.revision && reference.revision !== input.revision)) {
      throw new Error("Prepared action exact Fusion composition is stale or unavailable.");
    }
