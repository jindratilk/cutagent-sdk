...
Start the runtime first; it starts the local Free broker when needed:

```sh
cutagent runtime start --transport embedded_free
```

Keep the runtime terminal open, then run **Workspace > Scripts > CutAgentSDK** in DaVinci Resolve. Verify the live connection with `cutagent --json status`; require `data.status === "connected"` and `data.resolve === true`. Runtime readiness alone is not connection proof. The runtime stops only a broker it started itself.

It prints the discovery-file path used by SDK clients. When the client runs outside that shell environment, pass the absolute path to it:
