...
This runner module animates a verified Text+ size input on one exact composition. It reuses the supplied context; adapt the clip selection, tool ID, input ID, frames, and values from inspection rather than treating these names as universal.
...
export default async function animateTitle({
  ...
  if (!inputs.inputs.some((input) => input.id === "Size")) {
    throw new Error("Text1.Size was not found on the exact composition");
  }

  let revision = composition.revision;
  for (const [frame, value] of [[0, 0.05], [24, 0.09]]) {
    progress(`Setting Text1.Size at Fusion frame ${frame}`);
...
The public keyframe actions do not accept interpolation modes, easing presets, spline types, or tangent handles. For a constant input, the current runtime attempts to attach a Bézier spline before writing the first key. For an already animated input, it writes through the existing animation connection. Therefore, two successful keyframe writes prove keyed values, not a particular curve shape or tangent layout.
