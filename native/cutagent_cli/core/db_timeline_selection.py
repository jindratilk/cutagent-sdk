...
def require_exact_sdk_transition_batch_selection(
    ...
    *,
    ...
    from .sdk_transition_edges import require_clip_edges
    from .transition_db import normalize_transition_name

    selections = []
    edge_ids = set()
    for index, plan in enumerate(plans):
        custody = expected[index]
        selected = {"video": plan.get("video_item"), "audio": plan.get("audio_item")}
        if isinstance(custody, dict) and custody.get("kind") == "clip_edges":
            result = require_clip_edges(expected=custody, selected=selected, live_video=live_by_type["video"])
            item_id = result["video"].item_id
            if item_id in edge_ids:
                raise ValidationError("SDK clip-edge batch repeats a native target.")
            edge_ids.add(item_id)
            if plan.get("placement") != custody["placement"] or plan.get("scope") != "video" or plan.get("at_frame") is not None:
                raise ValidationError("SDK clip-edge plan changed its placement or scope.")
            if plan.get("duration_frames") != custody["durationFrames"] or plan.get("transition_name") != normalize_transition_name(custody["transitionType"]):
                raise ValidationError("SDK clip-edge plan changed its transition type or duration.")
        else:
            result = require_exact_sdk_transition_selection(conn, selected=selected, expected=custody, _live_by_type=live_by_type)
