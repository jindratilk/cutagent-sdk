...
def export_burnin_preset(conn, name: str, path: str) -> Dict[str, Any]:
    ...
    def reservation_permissions(observed, *, seal_empty=False):
        if os.name == "nt":
            from cutagent_cli.windows_file_security import private_file_permissions
            return private_file_permissions(output_path, observed, seal_empty=seal_empty)
        return not bool(observed.st_mode & 0o022)
    ...
    if output_path.exists() or output_path.is_symlink():
        ...
        if (
            ...
            or not reservation_permissions(reserved_stat, seal_empty=True)
        ):
            raise ValidationError(
    ...
    try:
        ...
        if result is not False and reserved_stat is not None:
            if native_path.exists() or native_path.is_symlink():
                native_cleanup = (
                    native_path.lstat(),
                    native_path.is_dir() and not native_path.is_symlink() and not getattr(native_path.lstat(), "st_file_attributes", 0) & 0x400,
                )
            ...
            if native_path.is_symlink() or (native_cleanup and getattr(native_cleanup[0], "st_file_attributes", 0) & 0x400):
                raise APICallFailed(
            ...
            if (
                ...
                or not reservation_permissions(current)
            ):
                raise APICallFailed(
                    "Burn-in preset export reservation changed before byte adoption.",
                    details={"path": path},
                )
            source_flags = os.O_RDONLY | getattr(os, "O_CLOEXEC", 0) | getattr(os, "O_NOFOLLOW", 0) | getattr(os, "O_BINARY", 0)
            try:
            ...
            with output_path.open("r+b") as destination:
                ...
                if (
                    ...
                    or not reservation_permissions(opened)
                ):
                    raise APICallFailed(
            ...
            if (
                ...
                or not reservation_permissions(adopted)
            ):
                raise APICallFailed(
        ...
        if native_path != output_path:
            ...
            if native_cleanup is not None:
                ...
                elif getattr(container_after, "st_file_attributes", 0) & 0x400 and native_path.is_dir():
                    native_path.rmdir()
                else:
                    native_path.unlink()
    if result is False:
...
def _activate_burnin_render_context(
    ...
    # Keep sibling captures in the custody root. Nesting them inside the
    # exported .drpx duplicates long preset names beyond native Windows limits.
    custody_root = Path(str(snapshot["preset_directory"]))
    runtime_name = f"CutAgent burn-in context {uuid.uuid4()}"
    verification_name = f"CutAgent burn-in verify {uuid.uuid4()}"
    runtime_bundle = custody_root / f"{runtime_name}.drpx"
    ...
    try:
        ...
        imported_path, imported_canonical, _settings = _export_render_context_preset(
            conn, runtime_name, custody_root / "burnin-imported"
        )
        ...
        _path, observed, _settings = _export_render_context_preset(
            conn, verification_name, custody_root / "burnin-verification"
        )
...
def load_burnin_preset(conn, name: str) -> Dict[str, Any]:
    ...
    # Embedded proxies expose callables even when the native owner lacks a
    # method. Project is the documented owner of project burn-in presets.
...
def delete_render_preset(conn, name: str) -> Dict[str, Any]:
    ...
    # The documented owner is Project. Embedded proxies expose callable method
    # handles even for methods absent from the underlying native object.
