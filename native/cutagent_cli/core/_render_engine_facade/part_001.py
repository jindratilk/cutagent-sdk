...
from ._render_engine.presets import (
    ...
    get_audio_render_codecs,
    get_audio_render_formats,
    get_render_codecs,
    get_render_formats,
    get_render_presets,
...
def _burnin_preset_settings_paths() -> List[Path]:
    ...
    if os.name == "nt":
        ...
        if not roaming or not Path(roaming).is_absolute():
            return []
        base = Path(roaming) / "Blackmagic Design" / "DaVinci Resolve"
        suffix = Path("Resolve Project Library/Resolve Projects/Settings/SlatePresetList.xml")
        return [base / "Support" / suffix, base / suffix]
    relative_candidates = (
