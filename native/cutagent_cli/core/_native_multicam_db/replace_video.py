        ...
        normalized.append(
            {
                ...
                **{key: entry[key] for key in ("duration_frames", "fps", "media_start_time") if key in entry},
            }
        )
    return normalized
    ...
    specs = [
        {
            ...
            "fps": target.get("fps"),
            "media_start_time": target.get("media_start_time"),
        }
...
def _build_verification(cursor: sqlite3.Cursor, *, mutation_result: dict[str, Any]) -> dict[str, Any]:
    rows = cursor.execute(
        ...
        SELECT
          ...
          item."In" AS InValue,
          item.MediaStartTime AS SourceStartSeconds,
    ...
    for row in rows:
        ...
        by_angle[int(payload["AngleIndex"] or 0)] = {
            ...
            "in": payload["InValue"],
            "media_start_time_seconds": float(payload["SourceStartSeconds"] or 0.0),
        }
        ...
        checks.append(
            {
                ...
                "ok": bool(actual and actual["source_media_id"] == replacement["source_media_id"]
                    and actual["start_frame"] == replacement["item_start_frame"]
                    and actual["duration_frames"] == replacement["duration_frames"]
                    and int(actual["in"] or 0) == replacement["source_start_frames"]
                    and abs(actual["media_start_time_seconds"] - replacement["media_start_time_seconds"]) < 1e-9),
...
def _resolve_or_import_video_targets(conn: Any, targets: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for target in _normalize_video_targets(targets):
        ...
        props = media_pool._clip_properties(resolved["clip"])
        source_fps = float(props.get("FPS") or getattr(conn, "fps", 24.0) or 24.0)
        source_tc = props.get("Start TC") or props.get("Start Timecode")
        if source_tc in (None, ""):
            raise ValidationError("Replacement video source start timecode is unavailable.", details={"source_path": path})
        updated = {
            **target,
            "clip_name": str(resolved.get("name") or target["clip_name"]),
            "source_path": str(resolved.get("source_path") or target["source_path"]),
            "fps": source_fps,
            "media_start_time": float(timecode_to_seconds(str(source_tc), source_fps)),
        }
