...
def _assert_owned(path: Path, *, reason: str) -> None:
    if os.name == "nt":
        from ..windows_file_security import private_file_permissions
        if not private_file_permissions(path, path.lstat(), owner_only=True):
            raise _validation("Project-library entry is not owned by the current Windows user.", reason=reason, path=str(path))
        return
...
def _dblist_path() -> Path:
    ...
    if sys.platform == "win32":
        ...
        return base / "Blackmagic Design" / "DaVinci Resolve" / "Preferences" / "dblist.conf"
...
def _parse_disk_registry(data: bytes, *, path: Path) -> list[dict[str, str]]:
    ...
    for index, raw_line in enumerate(text.splitlines(), 1):
        ...
        root = raw_root.strip()
        if sys.platform == "win32" and re.match(r"^[A-Za-z][\\/]", root):
            # Native Windows dblist.conf omits the drive colon because colon
            # is also the registry field separator.
            root = root[0] + ":" + root[1:]
        rows.append({"name": name.strip(), "root": root, "line": raw_line})
    return rows
...
def _with_bound_regular(path: Path, callback: Any, *, return_identity: bool = False) -> Any:
    ...
    try:
        ...
        if os.name == "nt":
            from .windows_bound_file import bound_sqlite_path
            with bound_sqlite_path(path, opened) as bound_path:
                result = callback(bound_path)
        elif os.name != "posix":
            raise _validation(
        ...
        if (after.st_dev, after.st_ino, after.st_size, after.st_mtime_ns, after_handle.st_ctime_ns) != (
            opened.st_dev,
            opened.st_ino,
            opened.st_size,
            opened.st_mtime_ns,
...
def _copy_bound_regular(
    ...
    *,
    ...
    captured_identity: list[_RegularIdentity] | None = None,
) -> dict[str, Any] | None:
    ...
    try:
        ...
        if sqlite_file:
            if os.name == "nt":
                from .windows_bound_file import bound_sqlite_path
                with bound_sqlite_path(source, opened) as bound_source:
            ...
            with os.fdopen(os.dup(descriptor), "rb") as source_handle, destination.open("xb") as destination_handle:
                ...
                if captured_identity is not None:
                    copied = os.fstat(destination_handle.fileno())
                    captured_identity.append((copied.st_dev, copied.st_ino, copied.st_size, copied.st_mtime_ns, copied.st_ctime_ns))
            result = None
...
def _new_internal_stage(kind: str) -> tuple[Path, tuple[int, int]]:
    if os.name == "nt":
        ...
        path = Path(tempfile.gettempdir()).resolve(strict=True) / f"cutagent-{kind}-{uuid4().hex}"
...
def _remove_internal_stage(path: Path, identity: tuple[int, int]) -> None:
    ...
    if os.name == "nt":
        from .windows_directory_ops import remove_owned_tree
...
def _exclusive_publish_directory(
    ...
    *,
    ...
    Windows holds the source and destination ancestors while publishing the
    source by handle with replacement disabled.
    """
    if sys.platform == "win32":
        from .windows_directory_ops import publish_owned_stage
        return publish_owned_stage(stage, stage_identity, target,
            bound_regulars=dict(bound_regulars or {}), published_regulars=published_regulars,
    ...
    if os.name == "nt":
        ...
        try:
            ...
            if (opened.st_dev, opened.st_ino) != (metadata.st_dev, metadata.st_ino):
                raise OSError("Project-library file changed while reading its identity.")
            metadata = opened
        finally:
            os.close(descriptor)
    return (
