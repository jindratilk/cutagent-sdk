...
from ..timeline_source_range import _compound_frame_value
...
def _preserved_time_value(value: Any) -> int | str:
    """Keep fractional frame cells byte-exact; never quantize protected audio."""
    text = str(value)
    if "|" not in text:
        return int(text)
    if _compound_frame_value(text) is None:
        raise APICallFailed("Invalid protected timeline item time value.")
    return text


def _shift_preserved_time(value: Any, delta: int) -> str:
    text = str(_preserved_time_value(value))
    whole, separator, fraction = text.partition("|")
    return f"{int(whole) - int(delta)}{separator}{fraction}"
    ...
    *,
    ...
    try:
        ...
        preserved_items_before = [
            {"item_id": str(item_id), "track_type": int(track_type), "start": _preserved_time_value(start), "duration": _preserved_time_value(duration)}
        ...
        if preserved_item_positions is not None:
            ...
            if set(actual) != set(expected) or any(
                _preserved_time_value(actual[item_id]["duration"]) != _preserved_time_value(expected[item_id]["duration"])
            ...
            for item_id, original in expected.items():
                cursor.execute(
                    ...
                    (_shift_preserved_time(original["start"], final_start_delta), item_id),
                )
