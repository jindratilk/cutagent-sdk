...
class _PersistedItemReader:
    ...
    def __enter__(self) -> "_PersistedItemReader":
        ...
        disk = sqlite3.connect(uri, uri=True, timeout=5.0)
        memory = sqlite3.connect(":memory:")
        try:
            # Native inspection calls can cause Resolve to save Project.db.
            # Capture one coherent snapshot, then release the disk read lock
            # before any of those native calls run.
            disk.execute("BEGIN")
            disk.backup(memory)
        except Exception:
            memory.close()
    ...
    *,
    ...
    return {
        ...
        "record": {
            ...
            "duration": record_duration,
            # Default API calls above project onto the video-frame grid. Keep
            # the independent GetStart/GetEnd/GetDuration(True) evidence for
            # audio verification; never infer it from those rounded values.
            "subframes": (authoritative_source or {}).get("record_subframes"),
        },
...
def write_timeline_inspection(conn: Any, output_path: str, *, force: bool = False) -> dict[str, Any]:
    ...
    if requested.is_symlink() or (requested.exists() and getattr(requested.lstat(), "st_file_attributes", 0) & 0x400):
    ...
    try:
        if os.name == "nt":
            ...
            candidate = destination.parent / f".{destination.name}.{uuid4().hex}.tmp"
            descriptor = create_private_file(candidate)
            temporary_name = str(candidate)
        else:
