...
class FusionAPI:
    ...
    def set_keyframe(self, tool_name: str, input_name: str, frame: int, value: Any) -> bool:
        ...
        try:
            # First, ensure the input has the native modifier required for its
            # data type. Point inputs cannot be animated by a BezierSpline;
            # Resolve requires an XYPath output instead.
            input_obj = self._find_input_by_id(tool, tool_name, input_name)
            kf = input_obj.GetKeyFrames()
            if not kf:
                attrs = input_obj.GetAttrs() or {}
                is_point = attrs.get("INPS_DataType") == "Point"
                modifier = self.comp.XYPath() if is_point else self.comp.BezierSpline()
                if not modifier:
                    modifier_name = "XYPath" if is_point else "BezierSpline"
                    raise APICallFailed(f"Failed to create {modifier_name} keyframe modifier")
                if input_obj.ConnectTo(modifier) is False:
                    raise APICallFailed("DaVinci Resolve rejected the keyframe modifier connection")
                connected_output = input_obj.GetConnectedOutput()
                if connected_output is None:
                    raise APICallFailed("Keyframe modifier connection was not established")
                if is_point:
                    output_attrs = connected_output.GetAttrs() or {}
                    modifier_tool = connected_output.GetTool()
                    modifier_attrs = modifier_tool.GetAttrs() if modifier_tool else {}
                    if (
                        output_attrs.get("OUTS_DataType") != "Point"
                        or (modifier_attrs or {}).get("TOOLS_RegID") != "XYPath"
                    ):
                        raise APICallFailed("Point input was not connected to an XYPath modifier")
            # Set input at specific frame (creates keyframe)
            if tool.SetInput(input_name, value, frame) is False:
                raise APICallFailed("DaVinci Resolve rejected the keyframe value")
            return True
        except Exception as e:
