...
def _fusion_inspection_target() -> dict[str, Any] | None:
    ...
    if (not isinstance(target, dict) or set(target) not in ({"timelineId", "timelineItemId"}, {"timelineId", "timelineItemIds"})
...
def _fusion_item_matches(native_id: str, target: dict[str, Any]) -> bool:
    ...
    return opaque_id in target["timelineItemIds"] if "timelineItemIds" in target else hmac.compare_digest(opaque_id, target["timelineItemId"])
