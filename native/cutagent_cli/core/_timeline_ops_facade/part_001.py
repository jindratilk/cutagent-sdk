...
def export_timeline(conn, path: str, format: str = "edl", *, single_file: bool = False) -> bool:
    """
    ...
    Args:
        ...
        single_file: Require a standalone FCPXML document for managed artifacts.
            Version 1.10 can produce a directory bundle; version 1.9 is a file.
    ...
    if single_file and format_key == "fcpxml":
        single_file_format = getattr(conn.resolve, "EXPORT_FCPXML_1_9", None)
        if single_file_format is None:
            raise ValidationError("DaVinci Resolve does not expose standalone FCPXML 1.9 export.")
        format_map["fcpxml"] = single_file_format
