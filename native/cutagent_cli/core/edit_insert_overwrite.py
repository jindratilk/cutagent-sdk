...
def _resolve_media_pool_item(
    ...
    *,
    ...
    return {
        ...
        "native_multicam": "multicam" in str(_clip_properties(item).get("Type") or "").casefold(),
        "bin": "/".join(path) or "/",
...
def _clip_info(
    *,
    ...
    track_index: int,
    timeline_fps: float,
    allow_source_extent_extension: bool = False,
) -> dict[str, Any]:
    ...
    if (
        ...
        and (
            ...
            or (allow_source_extent_extension and source_end == source_total_frames)
        )
    ):
        ...
        # Resolve accepts the one-past-extent endpoint when reconstructing an
        # existing tail and clamps its readback to the source's true extent.
        ...
        specs.append(
            {
                ...
                "clip_info": _clip_info(
                    ...
                    allow_source_extent_extension=True,
                ),
            }
        )
        ...
        specs.append(
            {
                ...
                "expected": {
                    ...
                    "native_multicam": track_type == "video" and plan["media"].get("native_multicam") is True,
                    "track_type": track_type,
                    "track_index": track_index,
...
def _match_expected(
    ...
    return [
        ...
        and (
            expected.get("name") is None
            or row.get("name") == expected.get("name")
            # Native multicam inserts append the active angle to the display
            # name. Exact source identity/ranges/track still match above/below;
            # existing edges and ordinary media never receive this exception.
            or (
                expected.get("native_multicam") is True
                and isinstance(row.get("name"), str)
                and row["name"].startswith(f"{expected['name']} - ")
                and bool(row["name"][len(expected["name"]) + 3:].strip())
            )
        )
        and (
...
def _public_plan(plan: dict[str, Any], *, dry_run: bool, verification: dict[str, Any] | None = None) -> dict[str, Any]:
    return {
        ...
        "source": {
            "name": plan["media"]["name"],
            "media_id": plan["media"]["media_id"],
            "native_multicam": plan["media"].get("native_multicam") is True,
