...
_DESCRIPTORS = (
    ...
    # Eight actions whose impact may span multiple objects. Preparation accepts
    ...
    _multi("timeline.clip_markers.batch", target_kinds=("clip",), linked=False),
...
if sum(row.family == "multi_target" for row in _DESCRIPTORS) != 8:
    raise RuntimeError(
        "Timeline/Version descriptor packet must contain eight multi-target Timeline actions."
    )
...
def public_timeline_version_descriptor_summary() -> dict[str, Any]:
    ...
    return {
        ...
        "counts": {"total": len(_DESCRIPTORS),
                   "stable": sum(row.family == "stable" for row in _DESCRIPTORS),
                   "multiTarget": sum(row.family == "multi_target" for row in _DESCRIPTORS),
                   "version": sum(row.family == "version" for row in _DESCRIPTORS)},
        "actions": actions,
    }
