    ...
    for name, artifact in binding.get("artifacts", {}).items():
        ...
        if name == "output":
            ...
            if (path.parent.is_symlink() or not isinstance(parent_identity, Mapping)
                    or not matches_carrier_identity(parent_stat, parent_identity)):
            ...
            if not path.is_file():
                ...
                stat = path.lstat()
                if not after and (not isinstance(identity, Mapping) or not matches_carrier_identity(stat, identity) or stat.st_size != 0):
