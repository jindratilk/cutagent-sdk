...
TIMELINE_ORDINARY_ACTION_IDS = (
    ...
    "cutagent.action.timeline.clip_markers.batch",
...
def _authority(context: Mapping[str, Any], action_id: str, value: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if action_id in {"cutagent.action.timeline.create", "cutagent.action.timeline.import"}:
    ...
    elif action_id in {"cutagent.action.timeline.clip_color.batch", "cutagent.action.timeline.clip_markers.batch", "cutagent.action.timeline.dolby.analyze"}:
        items = _item_rows(conn, str(timeline["id"]))
        requested = (
            [row["timelineItemId"] for row in value["updates"]]
            if action_id in {"cutagent.action.timeline.clip_color.batch", "cutagent.action.timeline.clip_markers.batch"}
        ...
        if action_id == "cutagent.action.timeline.clip_markers.batch":
            from .clip_marker_batch import lower_updates
            lower_updates(list(value["updates"]), items)
            selected = list({row["timelineItemId"]: row for row in selected}.values())
...
def _execute(context: Mapping[str, Any], command_id: str, value: Mapping[str, Any], prepared: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    elif command_id == "timeline.clip_markers.batch":
        from .clip_marker_batch import execute_batch
        raw = execute_batch(conn.timeline, native_timeline_id=documented_unique_id(conn.timeline),
                            updates=list(value["updates"]), rows=_item_rows(conn, value["timelineId"]), conn=conn)
...
def _project_result(_context: Mapping[str, Any], action_id: str, prepared: Mapping[str, Any], result: Any) -> Mapping[str, Any]:
    ...
    if command == "timeline.clip_markers.batch":
        return {"actionId": action_id, "markers": list(result["raw"]["markers"]), "timelineRevision": str(after["revision"])}
...
def _evidence(context: Mapping[str, Any], action_id: str, prepared: Mapping[str, Any], result: Any) -> Mapping[str, Any]:
    ...
    if command == "timeline.clip_markers.batch":
        target_matched = result.get("raw", {}).get("verified") is True and len(projected["markers"]) == len(value["updates"]) and all(
            observed["clipId"] == requested["timelineItemId"]
            and observed["recordFrame"] == requested["recordStartFrame"] + requested["offsetFrames"]
            and observed["offsetFrames"] == requested["offsetFrames"]
            and observed["durationFrames"] == requested["durationFrames"]
            and observed["name"] == requested["markerName"]
            and observed["note"] == requested["note"] and observed["color"] == requested["color"]
            for observed, requested in zip(projected["markers"], value["updates"]))
    if command == "timeline.clip_color.batch":
        target_matched = (
    ...
    before_active_id = before["active"]["timelineId"]
    after_active_id = after["active"]["timelineId"]
    activation_changed_ids = (
        {before_active_id, after_active_id}
        if before_active_id != after_active_id else set()
    )
    for timeline_id in common_structure_ids:
        before_tracks = before_structures[timeline_id].get("tracks", [])
        after_tracks = after_structures[timeline_id].get("tracks", [])
        if command in {"timeline.create", "timeline.import", "timeline.switch"} and timeline_id in activation_changed_ids:
