...
READ_ONLY_RESOLVE_CONSTANTS = frozenset({
    "EXPORT_EDL", "EXPORT_AAF", "EXPORT_DRT", "EXPORT_OTIO", "EXPORT_FCPXML",
    "EXPORT_FCPXML_1_8", "EXPORT_FCPXML_1_9", "EXPORT_FCPXML_1_10", "EXPORT_FCP_7_XML",
})
...
def _write_private_text(path: Path, text: str) -> None:
    if os.name == "nt" and not path.parent.exists():
        ...
        try:
            ...
            if not path.parent.is_dir() or path.parent.is_junction():
                raise
    else:
    ...
    if os.name == "nt":
        from .windows_file_security import create_private_file
...
def read_embedded_spool_response(path: Path) -> dict[str, Any] | None:
    ...
    try:
        if os.name == "nt":
            ...
            if not private_file_permissions(path, path.lstat(), seal_owned=True):
                return None
        else:
            path.chmod(0o600)
    except OSError:
...
class EmbeddedBridgeServer:
    ...
    async def _spool_round_trip(
        ...
        *,
        ...
        requires_acknowledgement = forwarded.get("requires_acknowledgement") is not False
        ...
        try:
            while True:
                ...
                now = time.monotonic()
                if requires_acknowledgement and not accepted and now >= accepted_deadline:
        ...
        try:
            ...
            if first.get("method") == "focus" and params.get("role") == "lua":
                if not self._auth_ok(first):
                    ...
                    return
                focused = await asyncio.to_thread(request_cutagent_focus_from_embedded_script)
                await self._send(
                    writer,
                    {
                        "id": first.get("id"),
    ...
    async def _register_lua_client(
        ...
        try:
            # Windows ShellExecute may wait for a cold desktop launch. Acknowledge
            # Lua first and keep the transport event loop available for commands.
            await asyncio.to_thread(request_cutagent_focus_from_embedded_script)
    ...
    def _validate_execute_request(self, request: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if op == "get_version":
            return None
        if op == "get_constant" and isinstance(params.get("name"), str) and params["name"] in READ_ONLY_RESOLVE_CONSTANTS:
            return None
        if op != "call":
    ...
    async def _execute_via_spool_lua(self, request: dict[str, Any]) -> dict[str, Any]:
        ...
        op = params.get("op")
        method = params.get("method")
        requires_acknowledgement = not (
            op in {"get_version", "get_constant"}
            or (op == "call" and isinstance(method, str) and method in READ_ONLY_RESOLVE_METHODS)
        )
        response = await self._spool_round_trip(
            {
                ...
                "method": "execute",
                "params": params,
                # Read-only calls are replay-safe and can return a single durable
                # completion. Mutations retain the accepted/remove/execute
                # handshake so a broker or script restart cannot replay them.
