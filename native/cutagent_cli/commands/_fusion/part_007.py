...
def _prepare_native_precise_holder_move(
    conn: Any,
    *,
    ...
    target_track_before = _target_track_preservation_snapshot(
        batch_context["before_rows"] if batch_context is not None else _enumerated_video_items(conn),
    ...
    def _writer(_connection: Any, cursor: Any, _session: Any) -> dict[str, Any]:
        ...
        if batch_context is not None:
            batch_context["reindex_tracks"].update(value for value in (source_track_id, target_track_id) if value)
        elif source_track_id:
            db_session.rebuild_track_item_indices(cursor, track_id=source_track_id)
        if batch_context is None and target_track_id and target_track_id != source_track_id:
    ...
    def _verifier(fresh_conn: Any, mutation_result: dict[str, Any], _session: Any) -> dict[str, Any]:
        ...
        all_rows = batch_context["after_rows"] if batch_context is not None else _enumerated_video_items(fresh_conn)
        ...
        target_track_after = _target_track_preservation_snapshot(
            [row for row in all_rows if (
                str(row["readback"].get("timeline_item_id") or "") not in batch_context["inserted_ids"]
                if batch_context is not None else row is not enumerated
            )],
            track_index=int(target_track),
        )
    ...
    return {"writer": _writer, "verifier": _verifier}


def _move_native_precise_holder_via_db(conn: Any, **kwargs: Any) -> dict[str, Any]:
    from ..connection import ResolveConnection
    from ..core import db_session

    prepared = _prepare_native_precise_holder_move(conn, **kwargs)
