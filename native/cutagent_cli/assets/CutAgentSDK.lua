...
-- Installed into Fusion/Scripts/Utility and run from Workspace > Scripts > CutAgentSDK.
...
if rawget(_G, "__CUTAGENT_SDK_BRIDGE_RUNNING") then return end
_G.__CUTAGENT_SDK_BRIDGE_RUNNING = true
...
local function log(message)
  ...
  if IS_WINDOWS then
    local appdata = os.getenv("APPDATA") or os.getenv("USERPROFILE")
    if not appdata then return end
    path = path_join(path_join(appdata, "CutAgentSDK"), "CutAgentSDK.log")
  else
    local home = os.getenv("HOME")
    if not home then return end
    path = home .. "/Library/Logs/CutAgentSDK.log"
  end
...
local function response_error_message(response)
  ...
  if type(response.error) == "table" then
    ...
    if code == "EMBEDDED_BRIDGE_OUTDATED" then
      return "CutAgentSDK.lua is out of date. Reinstall the DaVinci Resolve utility script from CutAgent."
    end
    if code == "EMBEDDED_BRIDGE_AUTH_FAILED" then
      return "CutAgent SDK could not connect. Restart the CutAgent script from Workspace > Scripts > CutAgentSDK."
    end
    return code .. ": " .. message
  end
  ...
  if type(result) == "table" and result.ok == false then
    ...
    return "CutAgent SDK could not start CutAgentSDK.lua: " .. tostring(result.status or "unknown")
  end
  return nil
end
...
local function write_spool_response(payload)
  -- Creating or loading a project can invalidate DaVinci Resolve's original
  -- Fusion application handle while this long-running Workspace script stays
  -- alive. Reacquire it before every durable spool response so the next SDK
  -- operation is still acknowledged after a project or timeline mutation.
  local fallback_fusion = nil
  if bmd and bmd.scriptapp then
    local ok_fusion, current_fusion = pcall(function() return bmd.scriptapp("Fusion") end)
    if ok_fusion and current_fusion
      and type(current_fusion.SetData) == "function"
      and type(current_fusion.SavePrefs) == "function" then
      fallback_fusion = current_fusion
    end
  end
  if not fallback_fusion and resolve and resolve.Fusion then
    local ok_fusion, current_fusion = pcall(function() return resolve:Fusion() end)
    if ok_fusion and current_fusion
      and type(current_fusion.SetData) == "function"
      and type(current_fusion.SavePrefs) == "function" then
      fallback_fusion = current_fusion
  ...
  local response_fusion = rawget(_G, "fusion") or rawget(_G, "fu") or fallback_fusion
  if not response_fusion or type(response_fusion.SetData) ~= "function" or type(response_fusion.SavePrefs) ~= "function" then
    return false, "response_fusion_unavailable"
  end
  ...
  local function persist_with(handle)
    if not handle or type(handle.SetData) ~= "function" or type(handle.SavePrefs) ~= "function" then
      return false, "response_fusion_unavailable"
    end
    local ok_set = pcall(function() handle:SetData(SPOOL_RESPONSE_KEY, encoded) end)
    if not ok_set then return false, "response_set_failed" end
    local ok_save, save_error = pcall(function() handle:SavePrefs(SPOOL_RESPONSE_PATH) end)
    pcall(function() handle:SetData(SPOOL_RESPONSE_KEY, nil) end)
    if not ok_save then return false, tostring(save_error or "response_save_failed") end
    return true, nil
  end
  local ok_write, write_error = persist_with(response_fusion)
  if not ok_write and fallback_fusion and fallback_fusion ~= response_fusion then
    return persist_with(fallback_fusion)
  end
  return ok_write, write_error
end
...
local function run_spool_bridge()
  if not fusion or type(fusion.SetData) ~= "function" or type(fusion.SavePrefs) ~= "function" then
    print("[CutAgent SDK] CutAgent SDK could not connect to DaVinci Resolve.")
    return
  end
  ...
  if not load_auth_config() then
    print("[CutAgent SDK] CutAgent SDK could not connect. Start the CutAgent SDK command, then run this script again.")
    return
  end
  ...
  print("[CutAgent SDK] CutAgent SDK is connected to DaVinci Resolve. Keep this script running while using CutAgent with DaVinci Resolve 21.1+ Free.")
  ...
  while true do
    ...
    if type(request) == "table" and type(request.id) == "string" and request.id ~= last_request_id then
      ...
      if expired then
        write_spool_response({
          ...
          error = {
            ...
            message = "The CutAgent SDK request expired before it could run.",
            details = { source = "CutAgentSDK.lua", connection = "file_spool" },
          },
        })
      elseif not request_auth_ok(request) then
        write_spool_response({
          ...
          error = {
            ...
            message = "CutAgent SDK could not verify this request.",
            details = { source = "CutAgentSDK.lua", connection = "file_spool" },
          },
        })
      ...
      elseif request.method == "execute" then
        local ready_to_execute = request.requires_acknowledgement == false
        if not ready_to_execute then
          ...
          if not accepted_written then
            print("[CutAgent SDK] CutAgent SDK could not verify this request, so it was not run.")
        ...
        write_spool_response({
          ...
          error = {
            ...
            message = "CutAgent SDK received an unsupported request.",
            details = { source = "CutAgentSDK.lua", method = request.method },
          },
        })
      end
    end
...
local function run_socket_bridge()
  ...
  if not hello_line then
    local message = "CutAgent SDK could not connect to CutAgentSDK.lua: " .. tostring(hello_err)
    print("[CutAgent SDK] " .. message)
    log(message)
    pcall(function() tcp:close() end)
    return
  ...
  if hello_failure then
    print("[CutAgent SDK] " .. hello_failure)
    log(hello_failure)
    pcall(function() tcp:close() end)
    return
  end

  print("[CutAgent SDK] CutAgent SDK is connected to DaVinci Resolve. Keep this script running while using CutAgent with DaVinci Resolve 20+ Free.")
  ...
  while true do
    ...
    if #ready > 0 then
          ...
          if not request_auth_ok(request) then
            send_line(tcp, attach_auth({
              ...
              error = {
                code = "EMBEDDED_BRIDGE_AUTH_FAILED",
                message = "CutAgent SDK could not verify this request.",
                details = { source = "CutAgentSDK.lua" },
              },
            }))
          else
            local ok_exec, result = pcall(execute, request.params or {})
      ...
      elseif read_err == "closed" then
        print("[CutAgent SDK] CutAgent SDK disconnected from DaVinci Resolve.")
...
local function run_http_bridge()
  ...
  print("[CutAgent SDK] CutAgent SDK is connected to DaVinci Resolve. Keep this script running while editing with DaVinci Resolve 20+ Free.")
