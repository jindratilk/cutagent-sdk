...
def wait_for_fresh_runtime_state(
    ...
    *,
    ...
    last_read_error: APICallFailed | None = None

    for attempt in range(1, attempts + 1):
        try:
            state = state_reader()
        except APICallFailed as exc:
            # Project close/reopen can briefly invalidate native API handles.
            # Retry only the read within the existing bounded poll, never the
            # mutation. A failed read is not proof that the project is closed.
            last_read_error = exc
        else:
            last_state = state
            if predicate(state):
    ...
    raise APICallFailed(
        ...
        details={
            ...
            "attempts": attempts,
            **({"last_read_error": {"code": last_read_error.code, "details": last_read_error.details}}
               if last_read_error is not None else {}),
        },
