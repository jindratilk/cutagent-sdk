...
from cutagent_cli.filesystem_safety import is_redirected_path
...
class FusionArtifactCustody:
    ...
    def _identity(context: Mapping[str, Any], source: str, *, stable_namespace: bool) -> tuple[Path, bytes, str, str]:
        path = Path(source)
        if not path.is_absolute() or not path.is_file() or is_redirected_path(path):
    ...
    def adopt(self, context: Mapping[str, Any], source: str, *, stable_namespace: bool = False) -> str:
        ...
        if is_redirected_path(root_input):
            raise InventoryValidationError("private artifact store root is unsafe")
        root = root_input.resolve()
        root.mkdir(parents=True, exist_ok=True)
        artifact_root = root / artifact_id
        if is_redirected_path(artifact_root) or (artifact_root.exists() and not artifact_root.is_dir()):
        ...
        if destination.exists():
            if (
                is_redirected_path(destination)
...
def _native_values_equal(actual: Any, expected: Any) -> bool:
    def numeric_map_sequence(value: Mapping[Any, Any]) -> tuple[Any, ...] | None:
        normalized = {str(key): item for key, item in value.items()}
        if not normalized or len(normalized) != len(value):
            return None
        keys = [str(index) for index in range(1, len(normalized) + 1)]
        if set(normalized) != set(keys):
            return None
    ...
    if isinstance(actual, Mapping) and isinstance(expected, (list, tuple)):
        ordered = numeric_map_sequence(actual)
        return ordered is not None and _native_values_equal(ordered, expected)
    if isinstance(actual, (list, tuple)) and isinstance(expected, Mapping):
        ordered = numeric_map_sequence(expected)
        return ordered is not None and _native_values_equal(actual, ordered)
...
def _native_tool_value(value: Any) -> Any:
    """Lower the public point object to Fusion's one-based native point map."""
    if isinstance(value, Mapping) and set(value) == {"x", "y"}:
        return {1: float(value["x"]), 2: float(value["y"]), 3: 0.0}
...
class FusionPreparedActionRuntime:
    ...
    def inspect(
        ...
        if command_id in _FILESYSTEM_COMMANDS:
            ...
            if any(
                (path.parent.exists() and (not path.parent.is_dir() or is_redirected_path(path.parent)))
    ...
    def execute(
        ...
        if command_id in _OFFLINE_COMMANDS:
            ...
            from .fusion_setting_batch import compiled_setting_items, execute_compiled_setting_batch
            ...
            compiled = compiled_setting_items(value, lowerings)
            if compiled is not None:
                native = execute_compiled_setting_batch(get_connection(require_project=True, require_timeline=True), compiled)
            else:
                common = {key: value[key] for key in ("projectId", "timelineId", "revision")}
                native = {
                    "items": [
        ...
        try:
            if command_id in _FILESYSTEM_COMMANDS:
                ...
                for raw_directory in reversed(checkpoint.created_directories):
                    ...
                    if directory.is_dir() and not is_redirected_path(directory):
                        directory.rmdir()
    ...
    def _resolve_template_path(template: str) -> Path:
        ...
        for directory in fusion_commands._template_directories():
            ...
            if path.is_file() and not is_redirected_path(path):
                return path.resolve()
    ...
    def _resolve_template_apply_path(template: str) -> Path:
        ...
        if not path.is_file() or is_redirected_path(path):
    ...
    def _invoke_handler(
        ...
        if command_id == "fusion.template.apply":
            ...
            if (
                ...
                or is_redirected_path(Path(template_path))
    ...
    def _execute_typed_tool_write(
        ...
        native_value = _native_tool_value(typed_value)
        ...
        if command_id in {"fusion.keyframe.add", "fusion.keyframe.set"}:
            if frame is None or api.set_keyframe(tool_name, input_name, frame, native_value) is False:
                raise APICallFailed("DaVinci Resolve rejected the exact typed Fusion keyframe write")
        elif api.set_tool_input(tool_name, input_name, native_value, frame) is False:
    ...
    def _verify_timed_tool_value(context: Mapping[str, Any], value: Mapping[str, Any]) -> bool:
        try:
            ...
            if keyframes and float(frame) not in {float(candidate) for candidate in keyframes.values()}:
                return False
            return _native_values_equal(
                tool.GetInput(input_name, frame), _native_tool_value(value["value"])
            )
