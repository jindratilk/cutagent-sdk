...
def _artifact_custody(
    ...
    for field_name in _ARTIFACT_FIELDS:
        ...
        if role == "source":
            ...
            observed = carrier_lstat(path)
            if (
                observed.st_size != 0
                or not matches_carrier_identity(observed, identity)
                or not private_file_permissions(path, observed, seal_empty=True)
            ):
...
def _revalidate_artifact_custody(records: Mapping[str, Any]) -> None:
    for record in records.values():
        ...
        if record.get("role") == "source":
            ...
            observed = carrier_lstat(path) if path.is_file() and not path.is_symlink() else None
            if not path.is_file() or _file_sha256(path) != (
                record.get("sha256"),
                record.get("byteLength"),
            ) or not isinstance(identity, Mapping) or observed is None or (
                not matches_carrier_identity(observed, identity)
            ...
            if not isinstance(identity, Mapping) or observed is None or (
                observed.st_size != 0
                or not matches_carrier_identity(observed, identity)
                or not private_file_permissions(path, observed)
            ):
