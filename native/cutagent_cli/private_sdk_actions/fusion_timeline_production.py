...
from ..private_file_security import private_file_permissions
...
class ProductionFusionPreparedActionRuntime(FusionPreparedActionRuntime):
    ...
    def _assert_reservations(context: Mapping[str, Any]) -> None:
        ...
        for record in records.values():
            ...
            current = carrier_lstat(path)
            if (
                path.is_symlink()
                or not stat.S_ISREG(current.st_mode)
                or current.st_size != 0
                or not matches_carrier_identity(current, identity)
                or not private_file_permissions(path, current, seal_empty=True)
            ):
    ...
    def _assert_managed_inputs(context: Mapping[str, Any]) -> None:
        ...
        for record in records.values():
            ...
            path = Path(path_value)
            current = carrier_lstat(path)
            if path.is_symlink() or not matches_carrier_identity(current, identity) \
            ...
            if stat.S_ISREG(current.st_mode):
                ...
                for child in path.rglob("*"):
                    ...
                    child_stat = carrier_lstat(child)
                    if child.is_symlink():
                ...
                for directory_entry in directory_entries:
                    ...
                    if not stat.S_ISDIR(child_stat.st_mode) or child.is_symlink() or not isinstance(expected_identity, Mapping) \
                            or not matches_carrier_identity(child_stat, expected_identity) \
                ...
                for entry in entries:
                    ...
                    if child.is_symlink() or not child.is_file() or not isinstance(child_identity, Mapping) \
                            or not matches_carrier_identity(child_stat, child_identity) \
...
def fusion_timeline_production_contribution_packet() -> Any:
    ...
    expected_fusion = 69
    if len(fusion) != expected_fusion or len(timeline) != 61 or len(descriptors) != expected_fusion + 61:
