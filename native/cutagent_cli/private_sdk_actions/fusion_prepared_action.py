...
def _public_digest(prefix: str, value: Any) -> str:
    """Match the Bridge's public, base64url SHA-256 identity projection."""

    encoded = base64.urlsafe_b64encode(hashlib.sha256(_canonical(value)).digest())
    return f"{prefix}f{encoded.decode('ascii').rstrip('=')}"


def _public_composition_revision(
    command_id: str,
    context: Mapping[str, Any] | None,
    value: Mapping[str, Any],
    state: Mapping[str, Any],
) -> str | None:
    """Project the exact post-state into the public live-composition domain."""

    if (
        context is None
        or command_id in _READ_COMMANDS
        or command_id == "fusion.comp.delete"
        or not all(key in value for key in ("timelineItemId", "compositionIndex"))
    ):
        return None
    binding = context.get("exactRequestBinding")
    revisions = binding.get("revisions") if isinstance(binding, Mapping) else None
    snapshot_revision = revisions.get("timeline") if isinstance(revisions, Mapping) else None
    timeline_item_id = value.get("timelineItemId")
    composition_index = value.get("compositionIndex")
    name = state.get("name")
    graph = state.get("graph")
    if (
        not isinstance(snapshot_revision, str)
        or not snapshot_revision
        or not isinstance(timeline_item_id, str)
        or not timeline_item_id
        or not isinstance(composition_index, int)
        or isinstance(composition_index, bool)
        or composition_index < 1
        or not isinstance(name, str)
        or not name
        or not isinstance(graph, Mapping)
    ):
        raise InventoryValidationError("Fusion public composition revision evidence is incomplete")
    composition_id = _public_digest(
        "fusion_comp_", {"timelineItemId": timeline_item_id, "index": composition_index}
    )
    graph_digest = _digest(graph)
    return _public_digest("revision_", {
...
def _public_projection(
    ...
    def copied_overwrite_truth() -> bool:
        ...
        return native_value
    revision_after = _public_composition_revision(command_id, context, value, after) or _revision(after)
    revision = {
...
class FusionPreparedActionDescriptor:
    ...
    def execute(self, context: Mapping[str, Any], prepared: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        public_result = _public_projection(self.command_id, value, private_result, context)
