...
from ..carrier_file_identity import carrier_lstat, matches_carrier_identity
...
from .waveform_source_custody import verify_waveform_project_source
...
class ResidualAvProductionAuthority(ResidualAvHandlerExecutionAuthority):
    ...
    def _artifact_bindings(self, descriptor, value, *, after: bool, context):
        ...
        bindings = []
        waveform_sources = context.get("privateBindings", {}).get("waveformSources")
        waveform_observed = None
        if waveform_sources is not None:
            if descriptor.action_id != "cutagent.action.audio.waveform_offset" or not isinstance(waveform_sources, (list, tuple)) or len(waveform_sources) != 2 or len(input_fields) != 2:
                raise ValidationError("Waveform source custody is not valid for this action.")
            if any(not isinstance(source, Mapping) for source in waveform_sources):
                raise ValidationError("Waveform source custody is malformed.")
            waveform_observed = self._inspect_collection(
                context, "audio.waveform.sources",
                mediaPoolItemIds=[source.get("sourceMediaPoolItemId") for source in waveform_sources],
                phase="verify" if after else "current",
            )
            if not isinstance(waveform_observed, (list, tuple)) or len(waveform_observed) != 2:
                raise ValidationError("Waveform source readback is incomplete.")
        input_index = 0
        output_index = 0
        for role in descriptor.artifact_roles:
            if role.startswith("managed_input"):
                if descriptor.action_id == "cutagent.action.edit.from_edl":
                    ...
                    stat = carrier_lstat(path) if path.exists() else None
                    if (
                        stat is None
                        or path.is_symlink()
                        or not path.is_file()
                        or not matches_carrier_identity(stat, identity)
                    ):
                        raise ValidationError(
                    ...
                    if waveform_sources is not None:
                        captured = waveform_sources[input_index]
                        if captured.get("resolvedPath") != value[input_fields[input_index]]:
                            raise ValidationError("Waveform source path changed after admission.")
                        artifact = verify_waveform_project_source(
                            captured, waveform_observed[input_index], project_id=context["project"]["projectId"],
                        )
                    else:
                ...
                if (
                    ...
                    or not matches_carrier_identity(path_stat, identity)
                ):
                    raise ValidationError(
                ...
                if after:
                    data = path.read_bytes()
                    after_stat = carrier_lstat(path)
                    if (
                        not matches_carrier_identity(after_stat, identity)
                        or not data
                    ):
                        raise ValidationError(
        ...
        if action_id.startswith("cutagent.action.edit."):
            ...
            if action_id == "cutagent.action.edit.transition.add":
                ...
                for index, transition in enumerate(requested):
                    ...
                    observed_rows = [*inserted_rows, *skipped_rows] if clip_edges else inserted_rows or skipped_rows
                    expected_starts = [expected_start]
                    expected_track = transition["outgoing"]["trackIndex"] if not clip_edges else transition["target"]["trackIndex"]
                    if clip_edges:
                        target = transition["target"]
                        expected_starts = []
                        if transition["placement"] in {"start", "both"}:
                            expected_starts.append(target["recordStartFrame"])
                        if transition["placement"] in {"end", "both"}:
                            expected_starts.append(target["recordEndFrame"] - transition["durationFrames"])
                    if len(observed_rows) != len(expected_starts) or any(
                        not isinstance(row, Mapping)
                        or row.get("track_type") != "video"
                        or row.get("track_index") != expected_track
                        or row.get("start") not in expected_starts
                        or row.get("duration") != transition["durationFrames"]
                        for row in observed_rows
                    ) or sorted(row.get("start") for row in observed_rows) != sorted(expected_starts):
                        raise ValidationError(
    ...
    def _resolve(self, action_id, context, value, *, phase):
        ...
        for key in _PATH_FIELDS:
            if isinstance(handler_input.get(key), str):
                ...
                elif action_id == "cutagent.action.audio.waveform_offset" and key in {"referencePath", "targetPath"} and context.get("privateBindings", {}).get("waveformSources") is not None:
                    # These artifacts were independently checked against fresh
                    # project-media and file custody earlier in this resolve.
                    # Only the two proven waveform input roles use this route.
                    role = "managed_input_reference" if key == "referencePath" else "managed_input_target"
                    exact = [item for item in artifacts if item.get("role") == role and item.get("resolvedPath") == handler_input[key]]
                    if len(exact) != 1:
                        raise ValidationError("Waveform handler has no independently verified source path.")
                    handler_input[key] = exact[0]["resolvedPath"]
                else:
