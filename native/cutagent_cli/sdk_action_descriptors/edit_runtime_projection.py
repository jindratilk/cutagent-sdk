...
def project_edit_result(
    ...
    if command == "edit.auto_subtitle":
        ...
        for index, transition in enumerate(requested_transitions):
            ...
            if transition.get("kind") == "clip_edges":
                native_rows = [*(native_result.get("inserted") or []), *(native_result.get("skipped_existing") or [])]
            target_id = (
                transition["outgoing"]["id"]
                if transition["placement"] == "end"
                else transition["incoming"]["id"]
            ) if transition.get("kind") != "clip_edges" else transition["target"]["id"]
                    ...
                    json.dumps(
                        {
                            ...
                            "index": index,
                            "frame": row.get("start") if transition.get("kind") == "clip_edges" else transition["editFrame"],
                            "revision": after["revision"],
                        },
                        sort_keys=True,
                    ).encode()
