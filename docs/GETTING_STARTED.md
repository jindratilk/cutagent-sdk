...
`setup` installs CutAgent SDK under `~/.local/share/cutagent-sdk` and creates `~/.local/bin/cutagent`. It preserves an existing command from the CutAgent desktop app or another installation. Choose a separate folder when both are installed. The generated launcher retains its configured state directory, so subsequent calls do not need `--state-dir` or a matching environment variable:
...
Start this runtime before activating **Workspace > Scripts > CutAgentSDK**. The command starts a missing local Free broker and stops only that owned broker when the runtime shuts down. It reuses an already-running authenticated broker without stopping it. After activating the script, run `cutagent --json status` and require `data.status === "connected"` and `data.resolve === true`. The runtime readiness message is not a live DaVinci Resolve health check.
...
Inspect the managed installation (this form is installation metadata, not DaVinci Resolve health):

```sh
cutagent status --json
```

Use `cutagent --json status` for live DaVinci Resolve health. These two existing command forms have different meanings; do not use installation metadata as connection proof.
