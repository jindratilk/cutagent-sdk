...
test("custom-state launcher preserves its installation without caller environment", async (t) => {
  if (process.platform === 'win32') return;
  const directory = await mkdtemp(join(tmpdir(), "cutagent-state-'quote-"));
  t.after(() => rm(directory, {recursive:true, force:true}));
  const state=join(directory,'state'), bin=join(directory,'bin');
  const setup=spawnSync(process.execPath,[entry,'setup','--skip-python','--free','--state-dir',state,'--bin-dir',bin,'--json'],{cwd:root,encoding:'utf8'});
  assert.equal(setup.status,0,setup.stderr);
  const result=spawnSync(join(bin,'cutagent'),['status','--json'],{encoding:'utf8',env:{...process.env,CUTAGENT_SDK_STATE_DIR:join(directory,'wrong-state')}});
  assert.equal(result.status,0,result.stderr);
  const actual=JSON.parse(result.stdout).data;
  assert.equal(actual.installed,true);
  assert.equal(actual.installation.installRoot,join(state,'versions','3.0.0'));
