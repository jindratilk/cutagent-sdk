...
test('existing native domain factories compose with original wrappers and local custody', async t => {
  ...
  try {
    ...
    for (const id of ['cutagent.action.timeline.clip_markers.batch','cutagent.action.timeline.marker.add','cutagent.action.timeline.marker.update','cutagent.action.timeline.marker.delete']) {
    ...
    const targeting = {CFFIXED_USER_HOME:'/isolated/worker/home', CUTAGENT_RESOLVE_UUID:'11111111-1111-4111-8111-111111111111', CUTAGENT_RESOLVE_HOST:'localhost', CUTAGENT_RESOLVE_PID:'12345'};
    const previous = Object.fromEntries(Object.keys(targeting).map(key => [key, process.env[key]]));
    try {
      Object.assign(process.env, targeting);
      const targetedEnv = buildCutAgentCliEnv({args:['--version']});
      for (const [key, value] of Object.entries(targeting)) assert.equal(targetedEnv[key], value);
    } finally {
