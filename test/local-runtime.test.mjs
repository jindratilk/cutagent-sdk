...
test('consuming discovery rotates its single-use bootstrap for the next SDK client', async t => {
  const runtime = await fixture(t);
  const discovery = await readFile(runtime.discoveryFile, 'utf8');
  const firstClient = await sdk.CutAgent.connect();

  let replacement = discovery;
  for (let attempt = 0; attempt < 100 && replacement === discovery; attempt += 1) {
    await new Promise(resolve => setTimeout(resolve, 10));
    replacement = await readFile(runtime.discoveryFile, 'utf8');
  }
  assert.notEqual(replacement, discovery);

  const secondClient = await sdk.CutAgent.connect({timeoutMs:1000});
  assert.equal(secondClient.connection.descriptor.distribution, 'standalone_local');
